...
import {
  FlatList, BackHandler, StyleSheet, View,
...
export default class Category extends React.Component {
  ...
  componentDidUpdate() {
    if (this.state.categoryId != this.props.navigation.getParam('data')) {
      this.state.categoryId = this.props.navigation.getParam('data');
      this.handleRefresh();
    }
  }
  ...
  handleNavigation(item, index) {
    let rv = false;
    for (var i = 0; i < Global.data.categories.length; i++) {
      if (Global.data.categories[i].parent == item.categoryId) {
        rv = true;
        i = Global.data.categories.length;
      }
    }
    if (rv == false) {
      Global.ProductData = item;
      Global.Product_PrentId = item.parentId;
      this.props.navigation.navigate('Products', {
        sourcePage: 'SubCategory', data: Global.ProductData, cateId: item.categoryId,
